...
typedef std::map<std::string, rd_kafka_topic_t *> TopicHash;
...
CnfCtx *loadCnf(const char *dir, char *errbuf)
{
  ...
  while ((ent = readdir(dh))) {
    if (strcmp(ent->d_name, ".") == 0 ||
        strcmp(ent->d_name, "..") == 0 ||
        strcmp(ent->d_name, "main.lua")) continue;
...
bool transform(LuaCtx *ctx, const char *line, size_t nline, std::string *result)
{
  ...
  if (!lua_isstring(ctx->L, 1)) {
    fprintf(stderr, "%s transform return #1 must be string\n", ctx->file.c_str());
    lua_settop(ctx->L, 0);
  }
...
void serializeCache(LuaCtx *ctx, const StringList &fields, StringPtrList *results)
{
  for (SSIHash::iterator ite = ctx->cache.begin(); ite != ctx->cache.end(); ++ite) {
    std::string *s = new std::string;
    if (ctx->withhost) s->append(ctx->main->host).append(1, ' ');
    if (ctx->withtime && ctx->timeidx != UNSET_INT) {
      s->append(fields[absidx(ctx->timeidx, fields.size())]).append(1, ' ');
    }
    s->append(ite->first);
...
bool aggregate(LuaCtx *ctx, const StringList &fields, StringPtrList *results)
{
  ...
  if (lua_pcall(ctx->L, fields.size(), 3, 0) != 0) {
  ...
  std::string anchor = lua_tostring(ctx->L, 1);
  
  if (!ctx->anchor.empty() && anchor != ctx->anchor) {
    serializeCache(ctx, fields, results);
    ctx->cache.clear();
    ctx->anchor = anchor;
  }

  if (!lua_isstring(ctx->L, 2)) {
    fprintf(stderr, "%s aggregate return #2 must be string\n", ctx->file.c_str());
    lua_settop(ctx->L, 0);
    return false;
  }
  std::string pkey = lua_tostring(ctx->L, 2);

  if (!lua_istable(ctx->L, 3)) {
    fprintf(stderr, "%s aggregate return #3 must be hash table\n", ctx->file.c_str());
    lua_settop(ctx->L, 0);
    return false;
  }
  lua_pushnil(ctx->L);  
  while (lua_next(ctx->L, 3) != 0) {
...
static bool lineAlign(LuaCtx *ctx)
{
  struct stat st;
  fstat(ctx->fd, &st);
  ctx->size = st.st_size;
  ...
  char *pos = (char *) memrchr(ctx->buffer, '\n', min);
  if (!pos) return false;

  lseek(ctx->fd, ctx->size - (ctx->buffer + min - pos), SEEK_SET);

  ctx->npos = ctx->buffer + min - pos;
  memmove(ctx->buffer, pos, ctx->npos);
  return true;
}

static const size_t WATCH_EVENT = IN_MODIFY | IN_DELETE_SELF | IN_MOVE_SELF;
...
inline void rmWatch(int wfd, int wd, LuaCtx *ctx, WatchCtxHash *wch)
{
...
inline LuaCtx *wd2ctx(WatchCtxHash &wch, int wd)
{
  WatchCtxHash::iterator pos = wch.find(wd);
  assert(pos != wch.end());
  return pos->second;
}
...
bool tail2kafka(LuaCtx *ctx)
{
  ...
  while (off <= st.st_size) {
    size_t min = std::min(st.st_size - off, (off_t) (MAX_LINE_LEN - ctx->npos));
...
{
  const size_t eventBufferSize = ctx->luaCtxs.size() * ONE_EVENT_SIZE;
  char *eventBuffer = (char *) malloc(eventBufferSize);
  
  ssize_t nn;
  while ((nn = read(ctx->wfd, eventBuffer, eventBufferSize)) > 0) {
    char *p = eventBuffer;
    while (p < eventBuffer + nn) {
      struct inotify_event *event = (struct inotify_event *) p;
      LuaCtx *lctx = wd2ctx(ctx->wch, event->wd);
      tail2kafka(lctx);
      if ((event->mask & IN_DELETE) || (event->mask & IN_DELETE_SELF)) {
        rmWatch(ctx->wfd, event->wd, lctx, &ctx->wch);
      }
...
bool processLine(LuaCtx *ctx, char *line, size_t nline)
{
  ...
  if (ctx->transform) {
    data = new std::string;
    rc = ctx->transform(ctx, line, nline, data);
  ...
  if (data) datas->push_back(data);
  
  if (datas->empty()) {
    delete datas;
  } else {
    OneTaskReq req = {ctx->topic, datas};
...
void uninitKafka(CnfCtx *ctx)
{
  for (TopicHash::iterator ite = ctx->rkts.begin(); ite != ctx->rkts.end(); ++ite) {
    rd_kafka_topic_destroy(ite->second);
  }
  rd_kafka_destroy(ctx->rk);
  rd_kafka_topic_conf_destroy(ctx->tconf);
...
void *routine(void *data)
{
  ...
  while (true) {
    ...
    TopicHash::iterator pos = ctx->rkts.find(req.topic);
    if (pos == ctx->rkts.end()) {
      fprintf(stderr, "invalid topic %s\n", req.topic.c_str());
      break;
    }
    rd_kafka_topic_t *rkt = pos->second;
